...
def polyopt(wrange=(9.e2,1.e5), dlw=2.1e-5, binary=False, strength=1e-4, inttab=1, \
  ...
  atom: str
      ...
      (default 'ap18')
  model: str
     set to a filename to ignore the temperatures and densities specified in
     the fort.2 file and use instead those from a model atmosphere given
     in that filename
  """
              ...
              for sfe in sfes: 
                for vmicro in vmicros:
                  ...
                  write2(lt,lrho,wrange, filename='opt.data', \
                         ...
                         inttab=inttab, ismodel=ismodel, zexclude=zexclude)
...
def grid_builder(config,  modeldir=modeldir):
    ...
    for entry in conf['grids']:
       ...
       if conf[entry]['type'] == 'marcs' or conf[entry]['type'] == 'kurucz':
         ...
         frun = open('run1.py','w')
         frun.write("import os\nimport glob\nimport numpy as np\nfrom synple import collect_regular_kurucz, polysyn, merge_slurm_parallel\n\n")
...
def mkgrid_irregular(synthfile=None, teff=True, logg=True, feh=True,  
           afe=False, cfe=False, vmicro=None, vrot=0.0, fwhm=0.0, vmacro=0.0, 
...
def write_synth(synthfile,p,d,hdr=None,irregular=False):
    ...
    for block in hdr:
      ...
      for entry in block: 
        ...
        elif all(char == ' ' for char in str(value)):
          fout.write(' '+entry + ' = ' + "'" + str(value) + "'" + '\n')
        else:
          fout.write(' '+entry + ' = ' + str(value) + '\n')
      fout.write(' /\n')
...
def write2(lt,lrho,wrange, filename='opt.data', dlw=2.1e-5, binary=False,strength=1e-4,inttab=1, ismodel=False, zexclude=None):
...
def bas(infile, synthfile=None, outfile=None, target=None, rv=None, ebv=None, 
    ...
    for file in infiles:
      ...
      if instr == 'DESI':
        ...
        hdu0.writeto(scrfile, overwrite=True)
...
def single_target_prep(wav,flux,ivar,rv,ebv,wavelengths=None):
    ...
    if np.abs(red) > 1e-7:
      ...
      ivar = ivar * (np.divide(ytmp,tmp,where=tmp>0, out= np.zeros_like(ivar)))**2
      flux = tmp
...
def wferrefits(root, path=None):	
  ...
  hdul=fits.HDUList(hdulist)
  hdul.writeto(root+'.fits', overwrite=True)
...
def alf_cal(rawspec):
    ...
    data = np.loadtxt("/home/callende/synple/config/alfosc-g19-response.dat")
    x = data[:,0]
    r = data[:,1]
