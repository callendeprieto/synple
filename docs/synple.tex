...
\section{Computing emergent intensities $I_{\nu}$}
...
\\
In [5]: for angle in range(10): plt.text(6160.2,max(inte[:,angle]),'$\mu=$'+"0.%d" % angle )}
\\
In [6]: plt.ylabel('I\$\_{nu}\$ (erg/s/cm2/Hz)')
\\
...
\noindent which will produce the graph shown in Fig. \ref{intensity}. Note that this option is not compatible with the broadening parameters {\tt fwhm}, {\tt vrot} or {\tt vmacro}.
