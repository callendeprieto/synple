...
\caption{Example of the computation of emergent specific intensities I$_{\nu}$ for various directions ($\mu = \cos \theta = 1$ the highest and $\mu = \cos \theta = 0.1$ the lowest). The units are in erg cm$^{-2}$ s$^{-1}$ Hz$^{-1}$ steradian$^{-1}$.
\label{intensity-fig}
}
\end{figure}
...
\\
In [5]: plt.ylabel('I\$\_\{nu\}\$ (erg/s/cm2/Hz)')
\\
